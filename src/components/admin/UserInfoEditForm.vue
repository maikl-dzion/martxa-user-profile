<template>
  <div class="blog-details-content">
    ...
    <h3 class="sidebar-title" style="font-style: italic; font-size: 17px">Данные пользователя : {{userInfo.username}}</h3><hr/>
    ...
    <div class="faq-form form-style" style="border:0px red solid; margin-bottom:20px;">
      <form id="user-general-info" @submit.prevent=""><div class="row">
         ...
         <template v-for="(item, fname) in userModel" >
                   ...
                   <template v-else >
                         ...
                         <div class="col-xs-8" >
                           ...
                           <template v-else >
                              ...
                              <template v-if="fname == 'username'" >
                                   <input v-model="userInfo[fname]"  :type="item.type"  required="true" :placeholder="item.label">
                              </template>
                              <template v-else-if="fname == 'role'">

                                  <select v-model="userInfo[fname]" class="form-control"
                                          style="margin-bottom: 10px; cursor:pointer; border-radius: 0px; border: 1px grey solid">
                                          <option v-for="role in roleList" :value="role.role_id"
                                          > {{role.name}} </option>
                                  </select>

                              </template>
                              <template v-else >
                                   <input v-model="userInfo[fname]"  :type="item.type" :placeholder="item.label" >
                              </template>
          ...
          <div v-if="userInfo.user_id" class="col-xs-12">
...
export default {
  ...
  methods: {
    ...
    async getRoleList () {
        const apiUrl = '/user/role/list'
        const roles = await this.send(apiUrl);
        this.roleList = roles;
    },
